...
echo please first specify a Seed to use and then the name of the experimental hyperparameter configuration to use, located at configs/
if [[ $# -eq 0 ]] ; then
...
CONF="configs/$2"
if [ ! -f "$CONF" ]; then
    echo 'The specified configuration was not found. be sure to specify it as the second argument!'
...
for FILE in configs/*/*;  
    ...
    if test -f $CONF; then
        ...
        cmd="pdm run python parallel_training.py $ARGS $EXP_SETUP_ARGS --config_file $CONF_FILE_SAVE_PARAMETER $3"
