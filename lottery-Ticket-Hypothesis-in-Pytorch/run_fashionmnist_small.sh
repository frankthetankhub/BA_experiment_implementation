...
name="fashionmnist_small"
out_file="out_lt_fashionmnist_small/$(date)/"
error_file="error_lth_fashionmnist_small/$(date)/"
for seed in 1 2 3 4 5; do echo $cluster_cmd -o $out_file$name$seed -e $error_file$name$seed $python_loc $MAIN $ARGS_TEST --seed $seed >> "runs from $(date) fashionmnist_small.txt"; done
